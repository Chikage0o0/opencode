...
- 当 shell 命令因为缺少一次性命令行工具而失败，或执行前已确认宿主环境缺少该工具时，不要先要求用户安装；优先使用 `nix shell nixpkgs#<package> -c <command> ...` 临时提供工具并执行原命令。
- 如果需要多个临时工具，优先在同一次 `nix shell` 中列出多个 `nixpkgs#<package>`；如果 `nix` 不可用、包名不确定、网络不可用，或该需求涉及持久服务/项目环境改造，简短说明原因并选择替代方案或询问用户。
