...
description: "Use when executing an existing implementation plan with independent tasks by using task subagents in the current session."
---
...
- **同一计划任务 + 同一子代理类型 => 必须复用该任务内上一次同类型 `task_id`。** 适用于 `implementer`、`spec-reviewer` 和 `code-reviewer`。
- **同一计划任务内的多次派遣不是新会话。** `NEEDS_CONTEXT`、`BLOCKED` 后补充上下文、重新派遣、规格审查/修复循环、代码审查/修复循环，都必须恢复该任务中对应类型的上一个 `task_id`。
...
- 如果当前任务标签未变化且同类型子代理已经存在，任何不传递该类型上一个 `task_id` 的行为都是对工作流的违反。
...
- 以“新审查者更独立”“复用会有确认偏误”“允许复用不是必须”为理由绕过任务内同类型 `task_id` 复用
